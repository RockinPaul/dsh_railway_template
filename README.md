...
| `dsh` | `DSH_GATE_PASSWORD` / `DSH_GATE_USER` | *(off)* / `dsh` | Optional HTTP basic auth at the edge, in front of DSH's own sign-in. |
...
1. Open the `dsh` service's **deploy logs** and find the line

   ```
   dsh web: http://127.0.0.1:7000/?token=…
   ```

2. Take the `?token=…` part and open **`https://<your-dsh-domain>/?token=…`** in a browser. DSH
   exchanges the token for a signed 30-day cookie and redirects to a clean `/`.
3. If you did not set `DEEPSEEK_API_KEY` at deploy time, paste it under *Settings → Models*.
4. Start a session. The LongMemory tools appear to the model as `mcp__longmemory__*` — recall,
   ingest, remember a decision, update task state, code graph and more.

The token is per process: a redeploy prints a new one. **Existing browser sessions keep working**,
because the cookie's signing secret lives in `$DSH_HOME/.credentials.yaml` on the volume. To revoke
every session, delete that record and redeploy.
...
- **The optional edge gate** adds HTTP basic auth in Caddy when `DSH_GATE_PASSWORD` is set. It hides
  the static shell from anonymous visitors and is a second, independent credential. `/up` bypasses
  it so the health check keeps working.
